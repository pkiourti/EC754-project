...
def train(numEpisodes=2000, startEpsilon=1.0, endEpsilon=0.001, epsDecayRate=0.9, targetReward=1000):
    if not os.path.exists('models/original' + str(numEpisodes)):
        os.makedirs('models/original' + str(numEpisodes))
    scores = []
    ...
    for episode in range(1, numEpisodes + 1):
        ...
        if episode % 100 == 0:
            ...
            agent.localNetwork.model.save('models/original' + str(numEpisodes) + '/model_{}.h5'.format(episode))
            agent.targetNetwork.model.save('models/original' + str(numEpisodes) + '/target_model_{}.h5'.format(episode))
...
scores = train(numEpisodes=episodes, targetReward=-110, epsDecayRate=0.9)
