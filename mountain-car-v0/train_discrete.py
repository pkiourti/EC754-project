...
def map_state_to_discrete_state(state, no_intervals):
    x, v = state
...
def train(numEpisodes=2000, startEpsilon=1.0, endEpsilon=0.001, epsDecayRate=0.9, targetReward=1000, no_intervals=128):
    if not os.path.exists('models/discrete' + str(numEpisodes)):
        os.makedirs('models/discrete' + str(numEpisodes))
    scores = []
    ...
    for episode in range(1, numEpisodes + 1):
        ...
        state = map_state_to_discrete_state(state, no_intervals)
        score = 0
        done = False
        while not done:
            action = agent.act(state, eps)
            nextState, reward, done, _ = env.step(action)
            nextState = map_state_to_discrete_state(nextState, no_intervals)
        ...
        if episode % 100 == 0:
            ...
            agent.localNetwork.model.save('models/discrete' + str(numEpisodes) + '/model_{}_{}.h5'.format(episode, no_intervals))
            agent.targetNetwork.model.save('models/discrete' + str(numEpisodes) + '/target_model_{}_{}.h5'.format(episode, no_intervals))
...
parser.add_argument('--no_intervals', dest='no_intervals', type=int, required=True)
...
print(agent.localNetwork.model.summary())
scores = train(numEpisodes=episodes, targetReward=-110, epsDecayRate=0.9, no_intervals=no_intervals)
